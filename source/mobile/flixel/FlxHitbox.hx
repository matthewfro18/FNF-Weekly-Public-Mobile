...
class FlxHitbox extends FlxSpriteGroup {
    ...
    public function new(?type:Int = 3) {
        ...
        var hitboxHeight:Int = FlxG.height;

        // Adjust hitbox calculations to account for scaling
        var cameraScrollX:Int = Math.floor(FlxG.camera.scroll.x * FlxG.scaleMode.scale.x);
        var cameraScrollY:Int = Math.floor(FlxG.camera.scroll.y * FlxG.scaleMode.scale.y);

        // Create and position hitboxes
        for (i in 0 ... keyCount) {
            var hitboxX:Int = cameraScrollX + (hitboxWidth * i);
            var hitboxY:Int = cameraScrollY;

            var button = createHitbox(hitboxX, hitboxY, hitboxWidth, hitboxHeight, hitboxColor[keyCount][i]);
            hitbox.add(button);
            array.push(button);
        }
    }

    public function createHitbox(x:Float = 0, y:Float = 0, width:Int, height:Int, color:Int):FlxButton {
    ...
    function createHintGraphic(Width:Int, Height:Int):BitmapData {
        ...
        if (guh >= 0.9) guh = ClientPrefs.controlsAlpha - 0.07;
